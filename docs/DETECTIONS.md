...
| `EDR-CONFIG-001` | canonical sequence | **Dark:** Hermes exposes no dedicated structured config-save outcome to this plugin; terminal output and generic file-write success do not prove that Hermes accepted or loaded a configuration change | **Producer-dependent** | Not evaluated by this importer |
| `EDR-CRON-001` | canonical sequence | **Live, outcome-gated:** exact built-in `cronjob` `create`/`update` calls emit `agent.automation.scheduled` only after a bounded authoritative success result | **Producer-dependent** | Not evaluated by this importer |
| `EDR-SCOPE-001` | canonical sequence | **Dark:** `post_approval_response` fires before Hermes persists session/permanent scope, so it proves a choice but not completed scope expansion | **Producer-dependent** | Not evaluated by this importer |
| `EDR-PERSIST-001` | canonical sequence | **Dark:** plugin emits no `agent.config.changed` event because no authoritative structured config-save outcome is available | **Producer-dependent** | Not evaluated by this importer |
...
Live rows still depend on producer-asserted timestamps and joins, exact typed attributes, successful durable ingestion, and a non-truncated correlation candidate set. Continuous generic sequence evaluation accepts exactly two-step rules and evaluates only matches containing the newly committed trigger in `O(rule_count * candidate_limit)` work; longer or duplicate-ID continuous rules fail closed before the transaction, while the explicit offline sequence API remains general. Cron outcome evidence depends on the exact built-in Hermes `cronjob` result contract; plugin/tool-registry compromise can forge it and oversized or ambiguous results intentionally fail dark. EXFIL uses deterministic `(observed_at_unix_ms,event_id)` ordering and trace-before-session precedence; equal timestamps are ordered by event ID. Safe malware-test markers require ASCII token boundaries, so prefix/suffix and Unicode case-folding near-matches do not set the malware indicator. The plugin's network classifier does not fully cover IPv6, indirect SDK/Python/cloud-client egress, `scp`, `rsync`, `ftp://`, or `s3://`; absence of a match is not proof of safety. Passive detection reports evidence but does not prevent an action. See [Hermes plugin telemetry](HERMES_PLUGIN_TELEMETRY.md#detection-limits) and [continuous ingestion operations](OPERATIONS.md#continuous-ingestion-operations).
