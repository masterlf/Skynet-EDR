...
## Current milestone: v0.6.0 Deployable Passive Detection

The v0.6.0 milestone carries forward the deployable alpha.3 passive path and the reusable, versioned threat-validation contract:
...
- a separate exact native Debian version plane (`0.6.0`) bound to package metadata and installed `dpkg` state;
...
No user-facing surface may claim guard mode before those controls are implemented and exercised end to end. Guard mode is not part of v0.6.0.
