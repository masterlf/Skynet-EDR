...
| `EDR-MALWARE-001` | Live, narrow; one real-host simulation | A fixed zero-I/O Hermes plugin tool exercises the default deferred dispatcher, pre/post hooks, authenticated local ingestion, correlation, API, and Risk Explorer. It accepts no arbitrary content and proves only the allowlisted malware-marker shape. |
