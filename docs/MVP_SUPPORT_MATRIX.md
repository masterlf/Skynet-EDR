...
| Autonomous Hermes enrollment | Unproven / blocked | A fail-closed transaction and deterministic boundary fixtures exist for Ubuntu 24.04 amd64/systemd plus Hermes 0.19.0, but no privileged live-host adapter or clean-host real-hook gate has passed. No host may yet be claimed autonomously `ENROLLED`; all other Hermes versions/platform cells are unsupported. |
