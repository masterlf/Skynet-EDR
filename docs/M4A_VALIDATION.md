...
The absolute output must not exist. Its existing ancestor chain is opened descriptor-relatively without following symlinks and must be root/effective-UID owned and not group/other writable. The validator retains descriptors for the checked parent and exact private mode-0700 sibling stage for its full lifetime. Report writes use `/proc/self/fd/<retained-stage-fd>`; sealing opens fixed allowlisted entries relative to that descriptor with `O_NOFOLLOW`; publication revalidates the parent entry against the retained stage inode and renames through the retained parent descriptor. The command uses `umask 077`, private temporary state and target directories, and Cargo offline mode.

The report publisher assumes an isolated job and no malicious concurrent process with the same effective UID and write authority over the private output parent. Retained descriptors prevent ordinary pathname or ancestor replacement from redirecting stage contents, but Linux pathname `rename` and cleanup are not inode-bound transactions: a same-UID writer can race the final namespace operation or substitute an internal entry between validation and a shell redirection. Such a writer already has the validator's authority and is outside this synthetic harness threat model. Do not run the validator in a shared same-UID writable directory. This mechanism is not a control against a compromised root account or compromised CI runner.
