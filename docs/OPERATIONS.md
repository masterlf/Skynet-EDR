...
The packaged continuous path accepts one length-prefixed `skynet.event.v0` event per authenticated AF_UNIX connection. Before persistence it applies the exact continuous-ingest event/source/trust/attribute projection; unknown, mistyped, raw-bearing, or non-allowlisted shapes are permanently rejected without an event, incident, or receipt. A successful transaction stores the projected event, evaluates the built-in sequence rules plus bounded `EDR-EXFIL-001` and `EDR-MALWARE-001` correlators, stores derived incidents before the receipt, then returns a versioned acknowledgement. After commit, each newly opened incident produces one compact redacted `skynet.alert.notice.v1` JSON line on daemon stdout for systemd/journald capture. It is not guard mode and never approves, delays, blocks, or rewrites an agent action.
...
The enrollment adapter is deliberately stricter than this general runbook. An explicitly authorized enrollment refresh restarts the complete `user@UID.service`, not only the gateway. That account-wide action interrupts every service in the user's systemd manager. The adapter then restarts `skynet-edr.service` and has one parent-created absolute 30-second monotonic budget for restart, identity proof, status/heartbeat reads, a 10-second non-mutating post-startup stability window, the single no-tool canary with a three-second connect cap and ten-second response cap but no post-send retry, parent persistence, and final assessment. It accepts only changed and finally stable manager, gateway, and daemon PID/start identities plus exactly one exact-schema fresh protocol-v3 source whose generation, independent plugin-process nonce, and kernel gateway PID/start identity agree; the nonce must also differ from the generation and attestation token. The canary command must return only its exact event-bound, token-free acknowledgement, then persist under its token-derived event ID on that same source with zero opened incidents; unrelated output or counter movement is not proof. Without explicit restart authorization enrollment remains `RELOAD_REQUIRED` and performs no user-manager restart.
...
When the local read-only API is enabled, `GET /api/status` includes an `ingestion` object. `state` is `disabled` when no listener was started. Active state is `healthy` only while the listener is live, at least one producer heartbeat is fresh, every configured `required_reported_roles` entry has at least one fresh/available/no-backlog instance, no fresh producer reports degraded transport or backlog, and no recent daemon degradation is active. It is `degraded` otherwise. Stale optional/transient `worker` or `unknown` instances remain visible during retention but do not poison health while a required gateway instance is fresh. With no configured requirements, no producers or all-stale producers remains degraded. Storage errors, frame timeouts, correlation overflow, alert-notice delivery errors, capacity rejection, listener/peer-credential errors, producer transport degradation, and non-zero backlog degrade the state.

```bash
curl --fail --silent http://127.0.0.1:8787/api/status
```

The object exposes bounded process-lifetime aggregates plus at most 64 complete source identities. Protocol-v3 sources are keyed by kernel-authenticated numeric UID plus fixed runtime role, exact lowercase 64-hex plugin generation, and an independent lowercase 64-hex runtime-instance nonce. The daemon binds the single accept-time credential PID to `SO_PEERPIDFD` and an opened `/proc/<pid>` start-tick proof, rejecting v3 when either cannot be revalidated. Status exposes only the bounded peer PID/start ticks as evidence rather than wire identity. Legacy v1, v2, and raw canonical inputs remain observable with `s3_eligible=false`. All source identities inactive for five minutes are evicted lazily before projection or producer insertion, so normal process restarts cannot consume the 64 slots forever; the map and retention state reset on daemon restart:
...
- outcomes: persisted, duplicate, event-ID collision, incident-integrity collision, correlation overflow, storage errors, and post-commit alert-delivery errors;
...
- separate daemon-observed last accepted and last committed hook-event timestamps/ages. Each exact v3 source also has `commit_sequence`/`events_persisted_total`; only a newly persisted event advances them, never a duplicate or collision. `hook_event_state` is `fresh`, `stale`, or `not_observed` from daemon receive time; it is not inferred from heartbeat traffic. `hook_event_freshness_affects_state=false` is explicit: an idle runtime with no expected user activity is not degraded solely because no hook event has occurred. Operators must compare event ages with known activity when investigating telemetry coverage.

The authenticated producer sends strict version-3 health and canonical-event transport envelopes. Duplicate object keys at any envelope or nested-event depth are rejected permanently; the nested event's exact raw JSON is passed unchanged to canonical validation. Unknown fields and non-canonical generation/nonce values are rejected. Legacy v1/v2 health and raw canonical events remain compatibility inputs but cannot establish S3 attribution. `/api/status` never projects socket/spool paths, event content, command text, or secrets. A report is stale 30 seconds after the daemon received it. Transient daemon errors degrade current state for the same 30-second health window; the cumulative counters and fixed-category last-error fields remain visible after current state recovers. `alert_delivery_errors_total` is a saturating process-lifetime aggregate; the source's fixed last-error category becomes `alert_delivery`. Runtime counters, source entries, liveness, and timestamps reset on daemon restart; durable events, receipts, incidents, collision fingerprints, producer fallback, and its checkpoint do not. Until a producer reports again after restart its transport is `unknown`, so status cannot claim end-to-end health.

### Local incident notices

The daemon writes notices only after SQLite commits and only for incidents inserted by that transaction. Duplicate event replay therefore emits no second notice. At most 64 notice projections are returned per transaction; exceeding that limit records `alert_delivery` degradation rather than growing memory or invalidating the commit. Each line contains exactly `schema_version`, `incident_id`, `severity`, `summary`, and `created_at_unix_ms`; the identifier is safe/routable and the already-redacted summary is capped at 512 Unicode scalar values. Event payloads, source identities, paths, tool arguments, and raw content are excluded.

```bash
journalctl -u skynet-edr.service -o cat | grep '"schema_version":"skynet.alert.notice.v1"'
```

Stdout/journald is a best-effort operational sink, not a durable outbox. A write failure cannot roll back the committed event, incident, or receipt and does not revoke the terminal producer ACK. Query SQLite through the local read-only CLI/API for authoritative incident state when `alert_delivery_errors_total` increases. There is no alert JSONL file, rotation, replay, or outbound delivery in this release.
