...
The enrollment adapter is deliberately stricter than this general runbook. An explicitly authorized enrollment refresh restarts the complete `user@UID.service`, not only the gateway. That account-wide action interrupts every service in the user's systemd manager. The adapter then restarts `skynet-edr.service` and has one absolute 15-second monotonic budget for restart, identity proof, status/heartbeat reads, and the single no-tool canary. It accepts only changed and finally stable manager, gateway, and daemon PID/start identities plus exactly one fresh protocol-v3 source whose generation, independent plugin-process nonce, and kernel gateway PID/start identity agree. Without explicit restart authorization enrollment remains `RELOAD_REQUIRED` and performs no user-manager restart.
