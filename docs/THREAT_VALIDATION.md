...
Validate-only evidence has status `not_tested`; it is never detection proof. `--scenario ID` rejects unknown IDs and limits reported results, although the bounded shared engine/producer gates still replay the complete corpus so cross-scenario regressions cannot be hidden.
...
The runner fails closed on duplicate JSON keys, duplicate IDs, unknown fields/categories/outcomes/execution modes, unsafe declarations, unknown scenario selections, unknown matrix statuses, omitted links, and live/skipped status drift.
