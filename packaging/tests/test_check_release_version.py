...
FIXTURE_FILES = (
    ...
    "docs/releases/v0.7.0-alpha.2.md",
    "README.md",
    "docs/ROADMAP.md",
    "docs/INSTALL.md",
...
class ReleaseVersionCheckerTests(unittest.TestCase):
    ...
    def make_cargo_fixture_self_contained(self) -> None:
        manifests = {
            ...
            "skynet-edr-cli": (
                "[dependencies]\n"
                'skynet-edr-core = { version = "0.7.0-alpha.2", path = "../skynet-edr-core" }\n'
            ),
            "skynet-edr-daemon": (
                "[dependencies]\n"
                'skynet-edr-core = { version = "0.7.0-alpha.2", path = "../skynet-edr-core" }\n'
                'skynet-edr-mcp = { version = "0.7.0-alpha.2", path = "../skynet-edr-mcp" }\n'
                "\n[dev-dependencies]\n"
            ),
            "skynet-edr-mcp": (
                "[dependencies]\n"
    ...
    def test_rejects_stale_current_ingestion_document_claims(self) -> None:
        ...
        current = current.replace(
            ...
            "v0.7.0-alpha.2 uses authenticated protocol-v3 live ingress for canonical events.",
        ).replace(
    ...
    def test_rejects_duplicate_nfpm_version_keys(self) -> None:
        ...
        for duplicate_key, duplicate_value in (
            ("version", "${SKYNET_EDR_DEB_VERSION:-0.7.0~alpha.2}"),
    ...
    def test_rejects_duplicate_dashboard_json_version_keys(self) -> None:
        ...
        for duplicate_version in ("0.7.0-alpha.2", "9.9.9"):
    ...
    def test_rejects_mismatch_in_each_internal_dependency(self) -> None:
        ...
        for crate, dependency in dependencies:
            with self.subTest(crate=crate, dependency=dependency):
                ...
                manifest.write_text(
                    original.replace(
                        f'{dependency} = {{ version = "0.7.0-alpha.2"',
    ...
    def test_accepts_prerelease_versions(self) -> None:
        for version in ("0.7.0-alpha.2", "1.0.0-rc.1", "2.3.4"):
    ...
    def test_checks_each_current_release_kind_authority_independently(self) -> None:
        ...
        with mock.patch.object(self.checker, "ROOT", self.fixture_root):
            self.checker.require_current_release_kind("0.7.0-alpha.2")
    ...
    def test_rejects_stale_authoritative_current_version_docs(self) -> None:
        for relative, current_marker in (
            ("SECURITY.md", "Skynet-EDR v0.7.0-alpha.2 is an installable prerelease"),
            ("SECURITY.md", "| `v0.7.0-alpha.2` prerelease |"),
            ("docs/README.md", "Current documentation structure target: v0.7.0-alpha.2."),
        ):
            with self.subTest(relative=relative):
                path = self.fixture_root / relative
                original = path.read_text(encoding="utf-8")
                path.write_text(
                    original.replace(current_marker, current_marker.replace("0.7.0-alpha.2", "9.9.9")),
                    encoding="utf-8",
                )
                try:
                    with self.assertRaises(SystemExit) as failure:
                        self.run_checker()
                    self.assertIn(
                        f"{relative} does not reference current release 0.7.0-alpha.2",
                        str(failure.exception),
                    )
                finally:
    ...
    def test_rejects_stale_current_product_surface_versions(self) -> None:
        for relative, current_marker in (
            ("docs/ARCHITECTURE.md", "ships in v0.7.0-alpha.2"),
            ("docs/CONCEPTS.md", "## Current v0.7.0-alpha.2 scope"),
            ("docs/HERMES_PLUGIN_TELEMETRY.md", "Skynet-EDR v0.7.0-alpha.2 ships"),
            ("docs/INTEGRATIONS.md", "the v0.7.0-alpha.2 integration index"),
            ("docs/INTEGRATIONS.md", "v0.7.0-alpha.2 live passive path"),
            ("docs/OPERATIONS.md", "the v0.7.0-alpha.2 operator index"),
            (
                "integrations/hermes/skynet-edr/README.md",
                "Skynet-EDR v0.7.0-alpha.2.",
            ),
            (
                "integrations/hermes/skynet-edr/README.md",
            ...
            with self.subTest(relative=relative, marker=current_marker):
                ...
                path.write_text(
                    original.replace(
                        current_marker,
                        current_marker.replace("0.7.0-alpha.2", "9.9.9"),
                    ),
                    encoding="utf-8",
                )
                try:
    ...
    def test_rejects_stale_install_enrollment_and_enroller_versions(self) -> None:
        cases = (
            ("docs/INSTALL.md", "DEB and RPM report `0.7.0~alpha.2`", "DEB and RPM report `9.9.9`"),
            ("docs/INSTALL.md", "Arch reports `0.7.0.alpha.2-1`", "Arch reports `9.9.9-1`"),
            ("docs/HERMES_ENROLLMENT.md", "Skynet-EDR plugin `0.7.0-alpha.2`", "Skynet-EDR plugin `9.9.9`"),
            ("packaging/scripts/skynet-edr-hermes-enroll.py", 'PAYLOAD_VERSION = "0.7.0-alpha.2"', 'PAYLOAD_VERSION = "9.9.9"'),
        )
    ...
    def test_rejects_internal_dependency_in_implicit_root_package(self) -> None:
        ...
        with workspace_manifest.open("a", encoding="utf-8") as manifest:
            manifest.write(
                ...
                "edition.workspace = true\n"
                "\n[dependencies]\n"
                'skynet-edr-core = { path = "crates/skynet-edr-core", version = ">=0.7.0-alpha.2" }\n'
            )
        ...
        self.assertIn("skynet-edr-root dependencies skynet-edr-core=>=0.7.0-alpha.2", str(failure.exception))
    ...
    def test_rejects_internal_dependency_in_implicit_path_member(self) -> None:
        ...
        (implicit_root / "Cargo.toml").write_text(
            ...
            "edition.workspace = true\n"
            "\n[dependencies]\n"
            'skynet-edr-core = { path = "../skynet-edr-core", version = ">=0.7.0-alpha.2" }\n',
            encoding="utf-8",
        )
        daemon_manifest = self.fixture_root / "crates/skynet-edr-daemon/Cargo.toml"
        daemon_manifest.write_text(
            daemon_manifest.read_text(encoding="utf-8").replace(
                "[dev-dependencies]\n",
                "[dev-dependencies]\n"
                'skynet-edr-implicit = { path = "../skynet-edr-implicit", version = "0.7.0-alpha.2" }\n',
            ),
            encoding="utf-8",
        )
        self.refresh_lock()
        ...
        self.assertIn(
            "skynet-edr-implicit dependencies skynet-edr-core=>=0.7.0-alpha.2",
            str(failure.exception),
        )
